...
usage() {
  cat <<'EOF'
Usage:
  infer_stage1_two_actions_10s.sh [EMPTY_WORK_DIR]

Set EMPTY_WORK_DIR as the first argument or through
LONG_LIVE_STAGE1_TWO_ACTION_WORK_DIR. The directory may not exist yet, or it
must already be empty. The script never removes an existing run directory.
EOF
}
...
if [[ -z "$work_dir" ]]; then
  usage >&2
  echo "error: provide a new work directory as the first argument or LONG_LIVE_STAGE1_TWO_ACTION_WORK_DIR" >&2
  exit 2
fi
...
if [[ ! -f "$metadata_path" ]]; then
  echo "error: two-action metadata is missing: $metadata_path" >&2
  exit 2
fi
...
cd "$project_root"
exec "$stage1_python" scripts/run_stage1_training_checkpoints_validation.py \
  --training-checkpoint "$training_checkpoint" \
  --metadata "$metadata_path" \
  --work-dir "$work_dir" \
  --num-latent-frames 64 \
  --allow-repeated-input-images \
  --comparison-mode prompt-style \
  --sampling-steps 50 \
  --guidance-scale 5.0 \
  --seed 1
