# Stage-2 C4 / W16 / S1 / K4 推理计时

此入口使用 `c4w16k4s1`，加载指定 Stage-2 checkpoint 的 Generator EMA 并合并 LoRA，以 BF16、CFG=1、每卡 batch=1 推理。`G120` 表示训练 checkpoint 的 generator update 120；`K4` 表示每个 chunk 的 4 步 denoising，两者相互独立。

固定配置为 4 个当前 latent、12 个历史 latent、1 个全局首图 sink。W16 包含当前 chunk，不包含独立的 sink，实际 self-KV 容量为 17 latent frames。单动作生成 24 个 future latent，共 6 个 chunk：4 步 DiT denoising，加上首次 sink preload 和每 chunk 的 clean recache，共 31 次 DiT 调用。双动作保留 sink 并重置局部缓存，共 61 次 DiT 调用。

## 运行

在 H100 服务器的项目根目录中执行，以下 `G120` 仅为示例，请使用实际完整且含 Generator EMA 的 checkpoint：
...
默认 GPU 0，单进程。`quick` 为 seed 1 的 4 个视频：单动作测试集 row 0、3，双动作测试集 row 0、4。`plan-quick` 执行资源路径、代码及 CPU 样本计划检查，不加载模型或初始化 CUDA；仍需模型文件路径、F25 manifest、完整 checkpoint、Python/torchrun 和 ffprobe 可用。

较完整的统计使用全部 6 个单动作、8 个双动作样本及 seeds 1、2、3、4，共 56 个视频：

```bash
bash infer_stage2_c4w16k4s1_timing.sh plan-formal G120
bash infer_stage2_c4w16k4s1_timing.sh formal G120
```

单动作视频为 96 帧、4 秒；双动作为 192 帧、8 秒，均为 24 FPS。分辨率遵循每行 metadata 的原始横竖屏尺寸：横屏 H×W=480×832，竖屏 H×W=832×480，计时按尺寸分组。quick 中单动作 row 0、3 为横屏，双动作 row 0、4 为竖屏；formal 中两个测试集都含横竖屏。每段动作将首图 latent 与 24 个 future latent 合并，VAE 解码 25 latent 为 97 帧后丢弃首帧。双动作的两个片段分别解码，最后拼接。
...
checkpoint 路径为 `$SNAPSHOT_ROOT/checkpoint_stage2_g000120`，需完整保留 `_SUCCESS`、manifest、provenance、EMA 和其余认证文件。C4 训练 G10/G20/G30 尚无 Generator EMA，应选用 G40 或之后已完成的 checkpoint。4 卡训练目录带 `_4gpus` 后缀，请相应覆盖 `STAGE2_WORK_ROOT`、`STAGE2_TRAIN_ROOT`；也可直接覆盖 `SNAPSHOT_ROOT`：
...
```bash
STAGE2_EARLY_CHECKPOINT=/actual/checkpoint_stage2_g000120 \
STAGE2_EARLY_OUTPUT=/actual/inference_timing_g000120_run1 \
  bash infer_stage2_c4w16k4s1_timing.sh quick
```

只有目录名为 `checkpoint_stage2_gNNNNNN` 时才自动提取训练步数；自定义目录名还需传入 `G120`。没有指定 step 或可提取的 checkpoint 路径时，入口会报错。
...
| `vae_decode_seconds` / `vae_decode_calls` | `vae.decode_to_pixel()`；单动作 1 次，双动作 2 次 |
| `video_postprocess_seconds` / `video_postprocess_calls` | 丢弃首帧、归一化和 clamp、双动作拼接，以及视频导出中的 CPU 传输、uint8 转换、MP4 编码与写入 |
...
输出沿用现有推理产物布局，并增加 `timing_summary.json` 和 `timing_samples.csv`。每个新生成样本会打印一行计时日志；trace 保留逐样本记录。汇总按 profile、单/双动作 dataset、分辨率、设备名称和计时方法分别统计，避免混合 4 秒与 8 秒视频；同型号 GPU 的不同本地卡号可合并统计。
...
```text
$STAGE2_TRAIN_ROOT/inference_early_g000120_infer_i2v_stage2_c4w16k4s1_timing_quick_<contract-hash>
```
