...
- `待执行 / 失败 / 通过` 必须对应真实状态。当前门禁未通过前，不进入下一批代码。

## 运行总览

| run_id | 日期 | 代码基线 | 内容 | 状态 |
|---|---|---|---|---|
| `H100-001` | 2026‑08‑08 | `639ba658a3a7f40bb792fbca4100877d2f7554df` | Batch 1 配置契约 | 待执行 |
...
全部通过后再通知 Codex 开始 Batch 2。任一项失败时保留整个 `STAGE2_RUN_DIR`，回传命令、完整日志和环境记录，不要自行放宽配置或测试。
